...
RegressionEnergyPatElectronProducer::RegressionEnergyPatElectronProducer( const edm::ParameterSet & cfg )
{
  ...
  sprintf(filename, "%s/src/EGamma/EGammaAnalysisTools/data/%s_V%d.root", descr, regressionInputFile_.c_str(), energyRegressionType_);
  if (fexist(filename))
    sprintf(filename, "http://cern.ch/sani/%s_V%d.root", regressionInputFile_.c_str(), energyRegressionType_);
  
  regressionEvaluator_->initialize(filename, type);
